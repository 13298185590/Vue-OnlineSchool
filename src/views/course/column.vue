<template>
  <div class="app-container">
    <el-button type="primary" icon="el-icon-edit" @click="handleCreate">新增专栏</el-button>
    ...
    >
      ...
      <el-table-column label="更新状态" class-name="status-col" width="90" prop="isend">
        
        <template slot-scope="{ row }">
          <span :style="{'color':row.isend?'':'red'}">{{row.isend?"已完结":"连载中"}}</span>
        </template>
      </el-table-column>
      <el-table-column
        prop="sub_count"
      ...
      >
        <template slot-scope="{ row, $index }">
          <el-button type="warning" size="mini" @click="goColumnDetail(row.id)"> 目录 </el-button>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" fullscreen="true">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="专栏内容" >
          ...
          >
          <el-button size="small" type="primary">上传专栏</el-button>
...
import { fetchList, updateColumn, deleteColumn,createColumn } from "@/api/column";
...
export default {
  ...
  methods: {
    ...
    //跳转到目录
    goColumnDetail(id){
      this.$router.push({path:'column_detail',query: {id:id}})
    }
  },
};
</script>
